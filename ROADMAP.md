...
- **wiktionary-and-wikisource** — Complete. Covers both Wiktionary and Wikisource: Wiktionary entry anatomy (language sections delimited by `----`, POS hierarchy, definition/example extraction, translation tables, pronunciation files, Wikidata lexemes), Wikisource ProofreadPage workflow (three-namespace Index→Page→Main system, quality levels 0-3, proofreading lifecycle), and cross-project API differences. Ships with `assets/wt_entry_parser.py` (importable Wiktionary parser for language sections, definitions, translations, and pronunciations), `assets/ws_proofread_checker.py` (importable Wikisource proofreading status checker with work stats and per-page quality), `assets/ws_text_extractor.py` (importable text extractor that strips header/footer markup from Wikisource pages), `scripts/wt-entry-summary.sh` (CLI tool to show Wiktionary entry structure), `scripts/ws-page-status.sh` (CLI tool to show proofreading progress), `scripts/ws-text-extract.sh` (CLI tool to extract cleaned page text), `references/wiktionary-entry-structure.md` (full entry anatomy reference), `references/wikisource-proofread-workflow.md` (three-namespace system and quality level reference), `references/sister-project-api.md` (API differences between Wikipedia, Wiktionary, and Wikisource), and `tests/test_wiktionary_wikisource.py` (41 tests). Depends on `wikimedia-api-access`, `wikimedia-commons`, `pywikibot`.
...
- **wikipedia-talk-page** — Complete. Updated June 2026. Now covers both legacy wikitext conventions (indentation, signatures, archiving, banners) and modern DiscussionTools (Reply Tool, New Topic Tool, Topic Subscriptions, Permalinks, Thank), the Usability Improvements rollout (topic containers, page frame, clear affordances), community gadgets (Convenient Discussions, Reply Link), Flow/Structured Discussions deprecation, the deep-nesting problem, and talk page etiquette. Ships with `references/discussion-tools-history.md` covering the 2019 Talk Pages Consultation, LiquidThreads, Flow, and DocumentMode vs. ThreadMode.
...
- **wikimedia-phabricator** — Complete. Covers navigating Wikimedia's Phabricator instance — searching tasks (basic, advanced, URL-based), understanding task status lifecycle (Needs Triage → Open → Stalled → Resolved → Closed), interpreting project tags and task hierarchies (parent/subtask, epics), filing bug reports (required fields, reproduction steps, conventions like no "me too" comments, security report process), and tracking what's in development via workboards and task relationships. Ships with a quick-reference card.
...
- **Code example standardization** — Systematic fixes to make code examples self-contained and copy-pasteable across all skills:
  - **Curl User-Agent headers:** Added `-H "User-Agent: ..."` to 16 curl commands in 3 skills (wikipedia-pagetriage-api, wikimedia-ml-services, wikipedia-en-article-audit) that previously required manual header setup
...
- **Cross-references between related skills** — Added to `wikidata` (→ ml-services, pageviews, categories, commons), `wikimedia-eventstreams` (→ ml-services, diffs, wikipedia-pagetriage-api), `wikipedia-page-anatomy` (→ templates, wikitext, citations, categories).
...
- **`wikipedia-talk-page` modernization** (June 2026) — Expanded from legacy wikitext conventions only to cover the full modern talk page landscape: DiscussionTools (Reply Tool, New Topic Tool, Topic Subscriptions, Permalinks, Thank), Usability Improvements (topic containers, page frame, clear affordances), community gadgets (Convenient Discussions, Reply Link), Structured Discussions/Flow deprecation awareness, the deep-nesting problem, and etiquette. Then trimmed 25% (420→314 lines) per the project's thin-skills design philosophy: removed non-actionable A/B test statistics, historical context, and verbose framing — moved to `references/discussion-tools-history.md` and URLs instead. The principle: a skill should contain only what an agent needs to *act* correctly, not what a human might want to *learn*.

- **`wikimedia-phabricator` skill** — New skill (June 2026, 238 lines) covering Wikimedia's Phabricator instance for bug tracking and project management. Covers: searching tasks (basic, advanced with query language, URL-based), understanding task status lifecycle (Needs Triage → Open → Stalled → Resolved → Closed), interpreting project tags and task hierarchies (parent/subtask, epics), filing bug reports with proper conventions (no "me too", security@ for vulnerabilities), and tracking what's in development via workboards. Ships with a quick-reference card. The 4th leanest skill in the project.

- **`research/future-skills/talk-pages-research-recommendations.md`** — Analysis document (June 2026) generated from systematic research into Wikipedia Talk page efficacy, the 2019 Talk Pages Consultation, DiscussionTools deployment, toxicity/retention studies, and community gadget landscape. Ranks 7 candidate skills by priority and effort, with detailed proposals for `wikimedia-community-health` (top remaining candidate), `wikimedia-structured-discussions-deprecation`, and smaller recommendations for cross-pollinating knowledge into 5 existing skills.
