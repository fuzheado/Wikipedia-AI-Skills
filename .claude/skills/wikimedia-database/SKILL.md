...
> `redirect` remain available on both clusters. Since **2026-09-14** the `x4` section is exposed on the Wiki
> Replicas, so the links hostnames return fresh data again (the s4 copies are no longer reachable from the
> replicas). Details:
