...
core tables frozen); **2026-09-14: `x4` was exposed on the Wiki Replicas as a separate
section** — queries return up-to-date information again, but the s4 copies of these
tables are no longer reachable from the replicas. The old s4 tables will eventually be
dropped. Tracked in
...
**Verified 2026-09-15** against the live replica: all 11 links tables plus `page` and
`redirect` (13/13) are reachable at `links.commonswiki.analytics.db.svc.wikimedia.cloud`
(MariaDB 10.11), and that hostname resolves to `x4.analytics.db.svc.wikimedia.cloud`.
